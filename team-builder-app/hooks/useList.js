...
export const useList=(initialValue,initialTeam)=>{
    const [storage, setStorage] = useStorage('team-builder-app',initialTeam)
    const [storageTemplate, setStorageTemplate] = useStorage('team-builder-app-template',initialValue);
    const [members, setMembers] = useState(storage);
        // ()=>{
            // only take attributes present in initialValue 
            // return storage.map((item)=>Object.keys(initialValue).reduce((acc,key,i)=>{
            //     acc[key] = item[key];
            //     return acc;
            // },{}));
    ...
    const [table, setTable] = useState(()=>{
        return storage.map((item)=>Object.keys(initialValue).map((key)=>item[key]));
    });
    const [editIndex, setEditIndex] = useState(null);
    const [template, setTemplate] = useState(storageTemplate);
    ...
    const saveHeaders=(insertArr,deleteArr,fieldNames)=>{
        ...
        setStorage(newMembers);
        setStorageTemplate(newTemplate);
        setIsEditing(false);
    };
